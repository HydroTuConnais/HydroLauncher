...
class ProcessBuilder {
  ...
  _constructJVMArguments112(mods, tempNativePath) {
    ...
    if (process.platform === "darwin") {
      args.push("-Xdock:name=AbyssoriaLauncher");
      args.push(
...
/**
 * Supprime tous les fichiers du dossier donné.
 * @param {string} dirPath Le chemin du dossier à vider.
 */
  emptyDir(dirPath) {
  ...
  /**
  * Copie tous les fichiers du dossier source vers le dossier destination.
...
async saveDropinModConfiguration() {
  ...
  */
  ...
  async syncRequiredModsBeforeLaunch() {
      const serv = (await DistroAPI.getDistribution()).getServerById(ConfigManager.getSelectedServer())
